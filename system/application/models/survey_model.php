...
class Survey_Model extends CI_Model {
  ...
  */
  function getSurveyDataForExport($surveyPrefix) {
   
$query = $this->db->query('SELECT '
        . $surveyPrefix.'_response_answers.response_id AS survey_session, '
        . $surveyPrefix.'_response_answers.created AS survey_response_date, '
        . $surveyPrefix.'_questions.question_text AS survey_question, '
        . $surveyPrefix.'_options.option_text AS survey_response_value, '
        . $surveyPrefix.'_responses.contact AS survey_contact_value, '
        . 'survey_users.email AS survey_target '
        . 'FROM '.$surveyPrefix.'_response_answers '
        . 'INNER JOIN '.$surveyPrefix.'_questions ON '.$surveyPrefix.'_questions.id = '.$surveyPrefix.'_response_answers.question_id '
        . 'INNER JOIN '.$surveyPrefix.'_options ON '.$surveyPrefix.'_options.id = '.$surveyPrefix.'_response_answers.option_id '
        . 'INNER JOIN '.$surveyPrefix.'_responses ON '.$surveyPrefix.'_responses.id = '.$surveyPrefix.'_response_answers.response_id '
        . 'INNER JOIN survey_users ON survey_users.id = '.$surveyPrefix.'_responses.user_id ;');
      ...
      foreach($surveyData as $question) {
        ...
        if(isset($_POST["ratings" . $question->id]) && !empty($_POST["ratings" . $question->id])) {
          ...
          if(!is_array($_POST["ratings" . $question->id]))
            $question->response = array($_POST["ratings" . $question->id]);
          else
            $question->response = $_POST["ratings" . $question->id];
          ...
          elseif(isset($_POST["ratings" . $question->id]) ) {
